...
def cmd_contacts(a): first, rest = one_shot("CONTACTS " + (a.n or "50")); print(first); print(rest.decode("utf-8", "replace"), end="")
def cmd_smsin(a): first, rest = one_shot("SMSIN " + (a.n or "20")); print(first); print(rest.decode("utf-8", "replace"), end="")
def cmd_drives(a): first, rest = one_shot("DRIVES"); print(first); print(rest.decode("utf-8", "replace"), end="")
...
def build_parser(prog="androremote adb"):
    ...
    s = sub.add_parser("contacts", help="List contacts"); s.add_argument("n", nargs="?"); s.set_defaults(fn=cmd_contacts)
    s = sub.add_parser("smsin", help="List inbox SMS"); s.add_argument("n", nargs="?"); s.set_defaults(fn=cmd_smsin)
    sub.add_parser("drives", help="List available storage drives").set_defaults(fn=cmd_drives)
